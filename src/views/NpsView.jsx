...
export default function NpsView({ summary, holdings, onDeleteHolding, onEditHolding, onOpenAddModal, onRefresh }) {
  ...
  const activeCount = rawNps.filter(h => (Number(h.quantity) || 0) > 0).length;
  const closedCount = rawNps.filter(h => (Number(h.quantity) || 0) === 0).length;
            ...
            <div className="flex items-center gap-4 sm:gap-6 relative z-10 shrink-0 flex-wrap sm:flex-nowrap justify-between xl:justify-end w-full xl:w-auto">
              <div className="text-right">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-0.5">
                  {statusFilter === 'active' ? 'Active NPS Valuation' : 'Total Realized Proceeds'}
                </span>
                <div className={`text-2xl sm:text-3xl font-black font-mono bg-clip-text text-transparent ${
                  statusFilter === 'active'
                    ? 'bg-gradient-to-r from-cyan-400 via-teal-300 to-indigo-300'
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5 pb-4 border-b border-slate-800/60">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-10 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-cyan-500"
                />
                {search && (
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-2.5">
                          <HoldingLogo 
                            ...
                            accentColor={isClosed ? '#06b6d4' : '#06b6d4'}
                          />
                          <div>
