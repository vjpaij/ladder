              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="[&>tr]:border-b [&>tr]:border-slate-800/40 text-xs">
                  {sortedHoldings.map((h, i) => {
                    ...
                    // Closed position derived values
                    const soldQty = Number(h.sell_qty) || Number(h.sold_qty) || Number(h.buy_qty) || 0;
                    const avgBuy = Number(h.avg_buy_price) || 0;
                    const investedVal = soldQty > 0 ? (soldQty * avgBuy) : (Number(h.investedValueINR) || 0);
                    const realizedPnl = Number(h.realized_pnl) || 0;
                        ...
                        <td className="sticky left-0 z-20 bg-slate-900/95 group-hover:bg-slate-900/95 py-3 px-3 border-r border-slate-800 min-w-[220px] transition-colors whitespace-nowrap">
                          <div className="flex items-center gap-2.5">
                            <HoldingLogo 
                              ...
                              accentColor={isClosed ? '#f59e0b' : '#10b981'}
                            />
                            <div>
                        ...
                        {statusFilter === 'closed' ? (
                          <>
                            ...
                            <td className="py-3 px-3 text-right font-mono whitespace-nowrap">
                              <div className={isRealizedPositive ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
                                {isRealizedPositive ? '+' : ''}{formatMoney(realizedPnl, true)}
                              </div>
                              <div className={`text-[9px] ${isRealizedPositive ? 'text-emerald-500/70' : 'text-rose-500/70'}`}>
                                {isRealizedPositive ? '+' : ''}{realizedPnlPct}%
                              </div>
                            </td>
                            ...
                            <td className="py-3 px-3 text-right font-mono whitespace-nowrap">
                              {Number(h.total_dividends || 0) > 0 ? (
                                <span className="text-amber-400 font-bold">
                                  {formatMoney(h.total_dividends, true)}
                                </span>
                              ) : (
                                <span className="text-slate-600">—</span>
                              )}
                          ...
                          <>
                            ...
                            <td className="py-3 px-3 text-right font-mono text-slate-400 whitespace-nowrap" title="Unrealized Average Buy Price">{formatMoney(h.avg_buy_price, true)}</td>
                            <td className="py-3 px-3 text-right font-mono whitespace-nowrap">
                            ...
                            <td className="py-3 px-3 text-right font-mono font-black text-slate-100 whitespace-nowrap">
                              {formatMoney(h.currentValueINR, true)}
                            </td>
                            {/* Unrealized P&L */}
                            <td className="py-3 px-3 text-right font-mono whitespace-nowrap">
                            ...
                            <td className="py-3 px-3 text-right font-mono whitespace-nowrap">
                              {Number(h.realized_pnl || 0) !== 0 ? (
                                <span className={Number(h.realized_pnl) >= 0 ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
                                  {Number(h.realized_pnl) >= 0 ? '+' : ''}{formatMoney(h.realized_pnl, true)}
                                </span>
                              ) : (
                                <span className="text-slate-600">—</span>
                              )}
