...
async function applyCharges() {
  console.log('=== STARTING PRE-MAY 12 2023 CHARGES UPDATE ===\n');
  ...
  const preExcelRows = excelRows.map((r, idx) => {
  ...
  console.log(`Total Excel rows before 2023-05-12: ${preExcelRows.length}`);

  // 4. Aggregate Excel charges by (Symbol, Date)
  const excelAgg = new Map();
  for (const r of preExcelRows) {
  ...
  console.log(`Total unique (Symbol, Date) combinations in Excel: ${excelAgg.size}\n`);
  ...
  // Re-fetch transactions for updated holdings to get exact sum
  ...
  console.log('=== SKIPPED DATE MISMATCH REPORT ===');
  console.log(JSON.stringify(skippedReport, null, 2));
}

applyCharges().catch(console.error);
